...
void renderPixel(int x, int y) {
	...
	image[x][y] = 1;
	image[y][x] = 1;	// Here we adding the pixel's symmetric counterpart
...
void rasterizeArc(int radius) {
	...
	while (y>x)
	{
		...
		else			// Here we selecting the value of dE which is closest to the circle
		{
			d += 2*(x-y) + 5;
			y--;
		}
		x++;
		renderPixel(x,y);		// Render Function
	}
